...
1. to Train on the entire dataset ***WITHOUT*** embeddings:
python fs_mol/maml_train.py ../fsmol_datasets/ --save-dir training_runs

2. to Train on the entire dataset ****WITH**** embeddings:
This is without the init_projection.
python fs_mol/maml_train.py ../fsmol_embedded_datasets/ --save-dir large_dataset_embedding_training_runs

3. This model was trained **with** the init_projection:
python fs_mol/maml_train.py ../fsmol_embedded_datasets/ --save-dir large_embedding_with_init_proj
...
def metatrain_loop(
    ...
    best_valid_epoch = 0
    # TODO(cfifty): this is a bit tricky...
    # TODO(cfifty): ah. they validate first for early stopping. and then again in each loop...
...
def run_metatraining_from_args(args):
    ...
    # TODO(cfifty): hack, slash, and burn it all!
    # Replace the node_features from 32 -> our 128 dimensional embedding.
    data_description.batch_features_shapes['node_features'] = (None, 128)
    model.build(data_description.batch_features_shapes)
    # Replace the learned embeddings with our frozen ones.
    ...
    # TODO(cfifty): these mfs create another model for the validation set.... WTF is wrong with them.
    # TODO(cfifty): meta learning *should not be* this convoluted.
    ...
    with tf.name_scope("valid"):
        ...
        valid_model.layers[-1]._initial_projection_layer = tf.keras.layers.Lambda(lambda x: x)
