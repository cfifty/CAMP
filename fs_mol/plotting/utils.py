...
TRAIN_SIZES_TO_COMPARE = [8, 16, 32, 64, 128, 256]
TRAIN_SIZES_TO_COMPARE = [8, 16, 32, 64, 128]
# TRAIN_SIZES_TO_COMPARE = [8]
...
def expand_values(
        ...
        train_sizes_to_compare=TRAIN_SIZES_TO_COMPARE
) -> pd.DataFrame:
  all_df = df.copy()
  ...
  return calculate_delta_auprc(all_df, model_summaries, train_sizes_to_compare)
...
def plot_by_size(
  ...
  categories = {x: i for i, x in enumerate(vals["GNN-MT"].index)}
  if highlight_class is not None:
    assert (
